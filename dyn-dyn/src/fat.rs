...
#[derive(Debug)]
pub struct DynDynFat<P: Deref> where P::Target: DynDynImpl {
    ptr: P,
    table: DynDynTable
}

impl<P: Deref> DynDynImpl for DynDynFat<P> where P::Target: DynDynImpl {
...
impl<P: Deref> DynDynFat<P> where P::Target: DynDynImpl {
...
impl<P: DerefMut> DynDynFat<P> where P::Target: DynDynImpl {
...
impl<P: StableDeref> DynDynFat<P> where P::Target: DynDynImpl {
    pub fn new(ptr: P) -> DynDynFat<P> {
        unsafe { Self::new_unchecked(ptr) }
    }
}

impl<P: Deref + Clone> DynDynFat<P> where P::Target: DynDynImpl {
    pub unsafe fn clone_unchecked(ptr: &Self) -> Self {
        DynDynFat { ptr: ptr.ptr.clone(), table: ptr.table }
    }
}

impl<P: Deref> Deref for DynDynFat<P> where P::Target: DynDynImpl {
...
impl<P: DerefMut> DerefMut for DynDynFat<P> where P::Target: DynDynImpl {
    fn deref_mut(&mut self) -> &mut Self::Target {
        self.ptr.deref_mut()
    }
}

impl<P: Deref + Clone> Clone for DynDynFat<P> where P::Target: DynDynImpl {
    fn clone(&self) -> Self {
...
mod test {
    ...
    fn test_get_table_cached() {
        ...
        assert_eq!(&EMPTY_TABLE_1.1[..] as *const _, ptr.get_dyn_dyn_table().traits as *const _);
        assert_eq!(1, num_table_calls.get());
    }
    ...
    fn test_clone_changes_table() {
        let num_table_calls = Cell::new(0);
        let ptr = DynDynFat::new(WeirdCloneBox(Box::new(TestStruct(&num_table_calls, &EMPTY_TABLE_1.1)))).clone();

        assert_eq!(2, num_table_calls.get());
        assert_eq!(&EMPTY_TABLE_2.1[..] as *const _, ptr.get_dyn_dyn_table().traits as *const _);
    }
}
